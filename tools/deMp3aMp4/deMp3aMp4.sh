...
show_menu() {
    ...
    echo "3. Limpiar nombres de archivos"
    echo "4. Configurar carpetas"
...
configure_folders() {
    ...
    mkdir -p "$dirMP3s" "$dirM4As" "$dirMP4_OUT" "$dirMP3_OUT"
}
...
# Limpiar nombres de archivos en todas las carpetas
clean_filenames_in_folder() {
    local folder="$1"
    shopt -s nullglob
...
clean_filenames() {
    ...
    clean_filenames_in_folder "$dirMP3s"
    clean_filenames_in_folder "$dirM4As"
    clean_filenames_in_folder "$dirMP4_OUT"
    clean_filenames_in_folder "$dirMP3_OUT"
