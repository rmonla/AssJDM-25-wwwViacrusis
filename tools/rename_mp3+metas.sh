...
# Verificar si eyeD3 está instalado
if ! command -v eyeD3 &> /dev/null; then
    echo "Error: Se requiere eyeD3 para modificar metadatos."
    echo "Instálalo con: pip install eyeD3"
    echo "O para instalación global: sudo pip install eyeD3"
    exit 1
fi
...
# Función para escribir metadatos con eyeD3
escribir_metadatos() {
    local archivo="$1"
    local titulo="$2"
    
    # Convertir caracteres especiales para eyeD3
    local titulo_encoded="$(echo "$titulo" | iconv -f UTF-8 -t ISO-8859-1//TRANSLIT)"
    local album_encoded="$(echo "$ALBUM" | iconv -f UTF-8 -t ISO-8859-1//TRANSLIT)"
    local artista_encoded="$(echo "$ARTISTA" | iconv -f UTF-8 -t ISO-8859-1//TRANSLIT)"
    
    # Escribir metadatos
    eyeD3 \
        --encoding utf-8 \
        --artist "$artista_encoded" \
        --album "$album_encoded" \
        --title "$titulo_encoded" \
        --year "$YEAR" \
        --genre "$GENRE" \
        "$archivo"
    
    # Verificar que se escribieron correctamente
    if eyeD3 "$archivo" | grep -q "$titulo_encoded"; then
        echo "Metadatos actualizados correctamente para $(basename "$archivo")"
    else
        echo "ERROR: Falló al escribir metadatos en $(basename "$archivo")"
        return 1
    fi
}
...
renombrar_y_actualizar() {
    for archivo in "${ARCHIVOS[@]}"; do
        ...
        if ! escribir_metadatos "$rutaNueva" "$titulo"; then
            # Segundo intento sin conversión de caracteres
            echo "Reintentando sin conversión de caracteres..."
            eyeD3 \
                --encoding utf-8 \
                --artist "$ARTISTA" \
                --album "$ALBUM" \
...
# Verificación final
echo ""
echo "=== VERIFICACIÓN FINAL DE METADATOS ==="
for archivo in "${ARCHIVOS[@]}"; do
    IFS='|' read -r -a datos <<< "$archivo"
    nuevoNombre="${datos[1]}"
    rutaNueva="$DIRECTORIO$nuevoNombre"
    ...
    if [ -f "$rutaNueva" ]; then
        echo -e "\n=== $nuevoNombre ==="
        eyeD3 --no-color "$rutaNueva" | grep -E "title|artist|album|year|genre"
    fi
done